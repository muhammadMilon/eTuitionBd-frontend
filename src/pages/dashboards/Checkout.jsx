...
const CheckoutForm = ({ application, applicationId, onSuccess }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter">Payment Success!</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[9px] uppercase opacity-50 block">Test Card</span>
                  <span className="font-mono bg-warning/20 px-2 py-0.5 rounded text-xs">4242 4242 4242 4242</span>
                </div>
                <div className="space-y-1">
                  <span className="text-[9px] uppercase opacity-50 block">Details</span>
                  <span className="font-mono bg-warning/20 px-2 py-0.5 rounded text-xs italic">12/30 • 123</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between px-1">
            <span className="text-xs font-black uppercase tracking-widest opacity-50">Secure Card Entry</span>
          ...
          <div className="p-5 rounded-2xl bg-white/5 border border-white/10 focus-within:ring-4 ring-primary/20 transition-all shadow-inner">
      ...
      <button
        type="submit"
        className="btn btn-primary btn-lg w-full rounded-2xl shadow-2xl shadow-primary/30 h-16 text-lg font-black tracking-tight"
      ...
      <p className="text-[10px] text-center opacity-30 flex items-center justify-center gap-2">
        <CreditCard size={12} />
    ...
    <div className="max-w-6xl mx-auto space-y-10 p-4 sm:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <motion.div 
          ...
          className="space-y-4"
        >
          <Link to="/dashboard/applications" className="group flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        >
          <div className="relative group">
            ...
            <div className="relative bg-slate-900 border border-white/10 rounded-[2.5rem] shadow-2xl shadow-black overflow-hidden">
              <div className="p-8 sm:p-12">
                <div className="flex items-center gap-4 mb-10 pb-6 border-b border-white/5">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-white italic tracking-tight">Payment Method</h2>
                    <p className="text-slate-400 text-sm">Transferring funds to Secure BD Escrow</p>
                  </div>
                </div>
        ...
        >
          <div className="bg-slate-900/50 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 sm:p-10 space-y-8 shadow-2xl">
            ...
            <div className="p-6 rounded-3xl bg-white/5 border border-white/10 flex items-center gap-5 group transition-all hover:bg-white/[0.08]">
              <div className="relative">
                ...
                <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-success rounded-full border-4 border-[#0f172a] flex items-center justify-center">
                  <CheckCircle size={10} className="text-white" />
                </div>
              </div>
              <div className="flex-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Assigned Tutor</p>
                <div className="font-bold text-white text-lg leading-none">{application.tutorId?.name || 'Tutor'}</div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6 pb-6 border-b border-white/5">
                <div className="space-y-1">
                  ...
                  <div className="text-white font-bold">{application.tuitionId?.subject}</div>
                </div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                    <Target size={12} /> Class
                  </div>
                  <div className="text-white font-bold">{application.tuitionId?.class}</div>
                </div>
              </div>

              <div className="space-y-4">
                <div className="flex justify-between items-center group">
                  <span className="text-slate-400 text-sm font-medium transition-colors group-hover:text-slate-300 italic">Monthly Tuition Salary</span>
                  <span className="text-white font-bold">৳{application.expectedSalary?.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center group">
                  <span className="text-slate-400 text-sm font-medium transition-colors group-hover:text-slate-300 italic">Service & Gateway Fee</span>
                  <span className="text-slate-500 font-bold line-through ml-2 text-xs">৳500</span>
                ...
                <div className="p-6 rounded-[2rem] bg-primary/10 border-2 border-primary/20 border-dashed mt-8 flex justify-between items-center group transition-all hover:bg-primary/20">
                  <div className="space-y-1">
                    ...
                    <div className="text-3xl font-black text-white italic tracking-tighter leading-none">৳{application.expectedSalary?.toLocaleString()}</div>
                  </div>
